...
def sort_jobs_by_time(jobs: list[SlurmJob], reverse: bool = True) -> list[SlurmJob]:
    ...
    Note:
        Time format is expected to be HH:MM:SS or similar.
        This function does basic string sorting which works for
        consistent time formats.
    """
    return sorted(jobs, key=lambda job: job.time, reverse=reverse)
